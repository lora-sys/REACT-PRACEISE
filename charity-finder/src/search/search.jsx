...
const Search=({onSearch})=>{//recieve prent  component's function,  making change in child component callled,
    ...
    const handleChange=(e)=>{
        const {name,value}=e.target;//use choose state to update the state
        setSearchCriteria(prev=>({
            ...prev,[name]:value
        }));//when user input ,update the state 
    };
    ...
    return(
        <section className='bg-white rounded-lg  shadow-md p-6 mb-8'>
            {
                loading && (
                    <div className='flex text-center h-8 bg-gray-400'>
        ...
        >
        <option value="">全部</option>
        {countries.map((country)=><option key={`served-${country}`} value={country}>{country}</option>)
        }
        </select>
        </div>
